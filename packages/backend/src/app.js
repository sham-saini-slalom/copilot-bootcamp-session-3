...
app.post('/api/tasks', (req, res) => {
  try {
    ...
    const priorityValue = priority || 'P3';
    if (!['P1', 'P2', 'P3'].includes(priorityValue)) {
    ...
    const result = stmt.run(title, description || '', due_date || null, priorityValue);
...
app.put('/api/tasks/:id', (req, res) => {
  try {
    ...
    const result = stmt.run(title, description || '', due_date || null, priorityValue, req.params.id);
